...
def main() -> None:
    ...
    for r in rows:
        ...
        for f in FIELDS:
            ...
            elif f == "required_filters":
                entry["required_filters"] = [x for x in v.split("|") if x]
    ...
    payload = {
        ...
        "columns_source": (
            "GET /v2/datasets/{dataset_key}/schema -- the database schema. Appendix M "
            "records that the API sometimes projects a column under a different name "
            "(twse_daily_price declares trade_date and returns `date`), so treat these "
            "as storage columns, not a guarantee of the response keys."
        ),
